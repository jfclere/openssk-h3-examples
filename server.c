                         ...
                         void *conn_user_data, void *stream_user_data)
{
    ...
    printf("on_end_stream for  %llu\n", (unsigned long long) stream_id);
    h3ssl->done = 1;
    return 0;
}

static int on_recv_settings(nghttp3_conn *conn,
                            const nghttp3_settings *settings,
                            void *conn_user_data)
{
    printf("on_recv_settings!\n");
    printf("on_recv_settings: %llu %lu %lu %lu %lu %d %d\n", settings->max_field_section_size, settings->qpack_max_dtable_capacity, settings->qpack_encoder_max_dtable_capacity,
                                                             settings->qpack_blocked_streams, settings->enable_connect_protocol, settings->h3_datagram);
    return 0;
}
...
/* process the h3 connection try to move it to close state */
int move_quic_server(nghttp3_conn *h3conn, struct h3ssl *h3ssl)
{
    ...
    while (!h3ssl->end_headers_received) {
        ...
        if (h3ssl->end_headers_received) {
            /* We have to build the response using the request */
    ...
    if (!h3ssl->data_sent) {
        ...
        /* XXX the resp and dr are in run_quic_server and fixed */
        if (nghttp3_conn_submit_response(h3conn, 0, h3ssl->resp, 2, &h3ssl->dr)) {
        ...
        return 5; /* wait for the close */
    }

    printf("move_quic_server %d %d\n", h3ssl->datadone,  h3ssl->close_done);
    if (!h3ssl->datadone) {
        return 5; /* Wait until all data are sent */
    }
    ...
    if (!h3ssl->close_done) {
        ...
        h3close(h3ssl, 0);
        h3ssl->close_done = 1;
        return 5; /* Wait for the closed state */
    }

    /* wait until really closed */
    if (!h3ssl->done) {
        int hasnothing;

        printf("WAIT DONE!!!! move_quic_server %d %d %d\n", h3ssl->datadone,  h3ssl->close_done, h3ssl->done);
    ...
    return 0; /* Done wait for ever until next one */
}
...
static int run_quic_server(SSL_CTX *ctx, int fd)
{
    ...
    for (;;) {
         ...
         */
        curconn = SSL_accept_connection(listener, 0);
        fprintf(stderr, "after SSL_accept_connection\n");
        fflush(stderr);
        if (curconn == NULL) {
            fprintf(stderr, "error while accepting connection\n");
            if (conn == NULL) {
                fprintf(stderr, "no nothing to process (no conn)\n");
                timeout = 0;
                continue;
            }
            if (h3conn == NULL) {
                fprintf(stderr, "no nothing to process (no h3conn)\n");
                goto err;
            }
            /* try to move the h3conn to the next step */
            timeout = move_quic_server(h3conn, &h3ssl);
            if (timeout == -1) {
                fprintf(stderr, "error while move_quic_server\n");
                goto err;
            } else if (timeout == 0) {
                /*
                ...
                SSL_free(conn);
                conn = NULL;
                h3conn = NULL; /* TODO alloc/free */
            }
        } else {
            if (conn != NULL) {
                fprintf(stderr, "Only one connection supported\n");
                goto err;
            }
            if (h3conn != NULL) {
                fprintf(stderr, "Previous http3 connection had problems...\n");
                goto err;
            }
            conn = curconn;
            ...
            callbacks.recv_settings = on_recv_settings;
            ...
            h3ssl.dr.read_data = step_read_data;
            h3ssl.resp = resp;
