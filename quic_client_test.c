...
static int jfc_send_stream(SSL *c_ssl, int ret, nghttp3_vec *vec)
{
    ...
    for (i=0; i<ret; i++) {
       ...
       SSL_write(c_ssl, vec[i].base, vec[i].len);
    ...
    for (;;) {
        ...
        if (c_connected && !c_write_done) {
            ...
            if (ret<0) {
                ...
                printf("sending %d on %d\n", ret, stream_id);
                jfc_send_stream(C_ssl, ret, vec);
                // nghttp3_conn_add_write_offset(conn, stream_id, fin);
            }
